...
class SqlAzureMigrationTask extends BaseMigrationTask {
    ...
    async getConnectionSecurity(scriptName) {
        if (scriptName.indexOf("LOGIN") != -1) {
            await this.clientSubscription.closeSubscriptionConnection();
            this.clientSubscription.connection = null;
            this.clientSubscription.connectionConfig.database = "master"
        }
         else {
            return await this.clientSubscription.openConnection();
        }
        return await this.clientSubscription.openConnection();
    }

    async runMigrationSecurity(info) {
            ...
            if (available.value) {
                ...
                try {
                    await cqlStatements
                        .map( stmt => 
                        (result) => {
                             return this.getConnectionSecurity(stmt)
                             .then( cn =>
                        ...
                        .then( () => 
                        console.log('[log] Execution scripts Security status SUCCESSFUL'+ "\n"))
                        .then( () => 
                        {
                    ...
                    return this.persistLog(available, info).then(() => Promise.reject(error));
            }
        } else {
